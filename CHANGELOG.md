...
- **Unique LAN / mDNS hostname** — Default device name is `altruist-insight-<id>` / `altruist-urban-<id>` (last 4 hex digits of the MAC) for DHCP/router listing and `http://altruist-…-<id>.local/`. Legacy plain names migrate on config load; user-chosen hostnames are kept. Hub nav uses functional zone names, not `altruist.local` as a label.
- **Hub web interface** — Device pages use a mobile-first **app shell** with four hub areas: **Local** (`/`), **Map** (`/social`), **Custom** (`/custom`), and **System** (`/advanced`). Desktop sidebar + mobile bottom tabs; breadcrumbs on inner views. Guest captive portal keeps the previous layout.
...
- **LoRa / Meshtastic UART (Urban C6)** — Prototype DM over CN4 UART (**TX GPIO22 / RX GPIO20**, **115200**, Heltec **Serial Module = PROTO**). Phone API session (`want_config_id`, heartbeat) plus **PKI unicast** to a configured destination (`!xxxxxxxx`), **PortNum 256**. Payload is **`core.v1.Message`** (not `SignedEnvelope`). Transport v1: `SINGLE` (`0x01`) if ≤220 bytes, else `FRAGMENT` (`0x41`, SHA-256[0:6], 213-byte chunks). Interval default **180 s**. USB CDC keeps firmware logs.
...
- **Hub density** — Content capped at **1400px** with aligned sidebar/content top offset; larger gaps between subsections; reading values **~34px**; short numeric inputs narrow (`~12ch`); Sleep/LEDs short fields in a content-sized pack; Sleep analytics split into light groups (night / morning / data).
- **Status header + Copy all** — Topbar title is **product — device id**; Insight shows Standalone / Urban paired tags. Three status chips: **Device** (Online/Offline + Wi‑Fi SSID, firmware, OTA link when available), wider **Send** (separate Map / Datalog rows with OK or Not send, `success/total` counts, per-API last send, Robonomics address tap-to-copy), and **IP** (IP + `.local` host, tap-to-copy). Chips stack full-width on narrow screens. Technical details keeps **Copy all**.
- **Web UI primary color** — Brand chrome and interactive accents use sensors.social blue **`#007bff`** (hover `#0062cc`): header (`.canvas` / app topbar), footer, buttons, links, focus rings, active tabs, and section bars. Topbar status chips use soft navy panels (`#1a3a66`) on the blue header.
- **Local Settings order** — Wi‑Fi credentials and Wi‑Fi in configuration mode sit together; the old **Firmware Version** block is renamed **System** / **Система** (timezone, region, auto-update).
- **OTA artifact `.version` files** — Build/publish scripts emit a human-readable version next to each `.bin` / `.md5` (optional on older mirrors; check still works on MD5 alone).
